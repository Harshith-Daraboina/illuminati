...
def check_scheduling_possibility(faculty, classroom, day, start_slot, duration, professor_schedule, classroom_schedule, timetable, TIME_SLOTS):
    ...
    for i in range(duration):
        ...
        # Check if slot is during morning break
...
def schedule_lunch_breaks(timetable, TIME_SLOTS):
    """Schedule lunch breaks for each day - any 2 consecutive half hours between 12:30-14:30"""
    for day_idx in range(len(DAYS)):
        # Find all potential lunch break slots (2 consecutive slots between 12:30-14:30)
        lunch_slots = []
        for slot_idx in range(len(TIME_SLOTS)-1):  # -1 because we need 2 consecutive slots
            start_time = TIME_SLOTS[slot_idx][0]
            next_start_time = TIME_SLOTS[slot_idx+1][0]
            if (time(12, 30) <= start_time < time(14, 30)) and (time(12, 30) <= next_start_time < time(14, 30)):
                lunch_slots.append(slot_idx)
        
        # Randomly select one lunch break slot
        if lunch_slots:
            lunch_start = random.choice(lunch_slots)
            lunch_end = lunch_start + 1
            
            # Mark lunch break slots
            for slot_idx in range(lunch_start, lunch_end+1):
                timetable[day_idx][slot_idx]['type'] = 'LUNCH'
                timetable[day_idx][slot_idx]['code'] = 'LUNCH'
                timetable[day_idx][slot_idx]['name'] = 'LUNCH BREAK'
                timetable[day_idx][slot_idx]['faculty'] = ''
                timetable[day_idx][slot_idx]['classroom'] = ''
                timetable[day_idx][slot_idx]['is_first'] = (slot_idx == lunch_start)
                timetable[day_idx][slot_idx]['duration'] = 2 if slot_idx == lunch_start else 0
                timetable[day_idx][slot_idx]['position'] = slot_idx - lunch_start
            
            logging.info(f"Scheduled lunch break on {DAYS[day_idx]} at {TIME_SLOTS[lunch_start][0].strftime('%H:%M')}-{TIME_SLOTS[lunch_end][1].strftime('%H:%M')}")
        else:
            logging.warning(f"Could not find suitable lunch break slots for {DAYS[day_idx]}")
...
def generate_all_timetables():
    ...
    for department in departments:
        ...
        for semester in semester_groups:
            ...
            # Schedule lunch breaks for each day BEFORE scheduling any courses
            schedule_lunch_breaks(timetable, TIME_SLOTS)
            ...
            lunch_fill = PatternFill(start_color="D3D3D3", end_color="D3D3D3", fill_type="solid") # Light Gray
                ...
                for slot_idx in range(len(TIME_SLOTS)):
                    ...
                    # Second priority: merge start points
                        ...
                        if activity_type == 'LUNCH':
                            ...
                            cell_fill = lec_fill
                            # Create cell content for lecture
                            cell_content = f"{activity['code']} {activity_type}\n{activity['name']}\n{activity['faculty']}\n{activity['classroom']}"
                        elif activity_type == 'LAB':
                            cell_fill = lab_fill
                            # Create cell content for lab
                            cell_content = f"{activity['code']} {activity_type}\n{activity['name']}\n{activity['faculty']}\n{activity['classroom']}"
                        else:  # TUT
                            cell_fill = tut_fill
                            # Create cell content for tutorial
                            cell_content = f"{activity['code']} {activity_type}\n{activity['name']}\n{activity['faculty']}\n{activity['classroom']}"
                        ...
                        try:
                            ...
                            logging.info(f"Successfully merged cells for {activity_type} on {day}")
                        except Exception as e:
